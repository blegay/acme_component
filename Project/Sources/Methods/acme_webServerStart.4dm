...
If (Not:C34(WEB Is server running:C1313))
	...
	If (acme_certCurrentGet (->$vt_cert))  // we have a certificate
		acme__moduleDebugDateTimeLine (4;Current method name:C684;"certficate found")
	...
	Else   // no certificate => no https
		acme__moduleDebugDateTimeLine (2;Current method name:C684;"no certficate found")
	...
	acme__moduleDebugDateTimeLine (Choose:C955($vb_started;4;2);Current method name:C684;"web server started. "+Choose:C955(ok=1;"[OK]";"[KO]"))
